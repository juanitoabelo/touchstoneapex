...
const bar = new ProgressBar(`Transcoding Videos [:bar] :current/:total :elapsed secs :percent`, {
  total: 0,
  width: 30
});
...
const getVideoDimensions = async file => {
  if (process.env.NODE_ENV !== `test` && videoDimensionCache.has(file.internal.contentDigest)) {
...
const processFile = async (file, jobs, cb, reporter) => {
  ...
  jobs.forEach(async job => {
    ...
    clonedPipeline.output(job.outputPath).on('end', onFinish).on('error', onFinish).run();
...
async function queueVideoTranscode({
  file,
  options = {},
  reporter
}) {
  const fileExtension = options.fileExtension;
  const argsDigest = crypto.createHash(`md5`).update(JSON.stringify(Object.assign({}, options, {
  ...
  const filePath = path.join(process.cwd(), `public`, `static`, videoSrc); // Create function to call when the image is finished.
  ...
  const job = {
    ...
    outputPath: filePath // queue up the job

  };
  queueJob(job, reporter); // TODO: Do we need a path prefix?
...
async function transcode({
  ...
  if (!options.pipelines || options.pipelines.length < 1) {
    reportError(`FFMPEG Options has no pipelines`, new Error('gatsby-plugin-ffmpeg has no pipelines!'), reporter);
    return;
  }

  const videos = await Promise.all(options.pipelines.map(pipeline => queueVideoTranscode({
    file,
    options: pipeline,
    reporter
  })));
